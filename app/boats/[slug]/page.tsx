...
import { Check, ChevronRight, Shield } from '@/components/icons';
import { PHOTO_SIZES, photoPath } from '@/lib/photo-paths';
import { boatBySlug, departuresOnBoat, tripsOnBoat } from '@/lib/queries';
...
export async function generateMetadata({ params }: PageProps<'/boats/[slug]'>): Promise<Metadata> {
  ...
  if (!boat) return {};
  return { title: `${boat.name} · ${boat.type}`, description: boat.blurb };
}

export default async function BoatDetailPage({ params }: PageProps<'/boats/[slug]'>) {
  const { slug } = await params;
  const boat = boatBySlug(slug);
  if (!boat) notFound();

  const trips = tripsOnBoat(boat.slug);
  const deps = departuresOnBoat(boat.slug).slice(0, 4);
  ...
  for (const c of boat.cabinTypes) {
  ...
  const deckPlan = [
    ...decks.map((d) => ({ name: d.name, detail: `${d.cabins.join(', ')}.` })),
    { name: 'Shared spaces', detail: `${boat.facilities.slice(0, 4).join(', ')}.` },
  ];
    ...
    <>
      <section className="relative isolate flex min-h-[72vh] items-end overflow-hidden bg-ink">
        ...
        <div className="relative mx-auto w-full max-w-8xl px-5 pb-12 pt-28 sm:px-6 lg:px-8 lg:pb-16">
          ...
          <div className="mt-8 max-w-3xl">
            <p className="font-mark text-eyebrow uppercase text-white/70">{boat.type}</p>
            <h1 className="mt-4 font-display text-4xl font-light leading-[1.04] tracking-tight text-white sm:text-5xl lg:text-6xl">{boat.name}</h1>
            <p className="mt-5 max-w-xl font-display text-lg font-light leading-relaxed text-white/85 sm:text-2xl">{boat.tagline}</p>
          </div>
          <div className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <Link
              href={routes.charter({ boat: boat.slug })}
              className="inline-flex items-center rounded-full border border-white/35 px-6 py-4 font-mark text-[12px] uppercase tracking-[0.14em] text-white transition hover:bg-white hover:text-ink-700"
            >
      ...
      <section className="mx-auto max-w-8xl px-5 py-14 sm:px-6 lg:px-8 lg:py-20">
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr] lg:gap-16">
          <div>
            <p className="font-mark text-eyebrow uppercase text-flame">Her story</p>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3">
              {boat.gallery.map((g, i) => (
                <div key={i} className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-ink">
                  <PhotoSlot ph={g} src={photoPath.boatGallery(boat.slug, i)} alt={boat.name} sizes={PHOTO_SIZES.gallery} />
                </div>
              ))}
            </div>
          </div>
              ...
              <ul className="mt-4 space-y-2.5">
                {boat.safety.map((s) => (
                  <li key={s} className="flex gap-3 text-sm leading-relaxed text-ink/80">
                    <Shield className="mt-0.5 h-4 w-4 shrink-0 text-mist" aria-hidden="true" />
                    <span>{s}</span>
                  </li>
                ))}
              </ul>
              <Link href={routes.policies('safety')} className="mt-5 inline-block font-mark text-[11px] uppercase tracking-[0.16em] text-flame-600 underline underline-offset-4">
                Full safety policy
              </Link>
            </div>
          </aside>
        ...
        <div className="mx-auto max-w-8xl px-5 py-14 sm:px-6 lg:px-8 lg:py-20">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div className="max-w-xl">
              <p className="font-mark text-eyebrow uppercase text-flame">Cabins</p>
              ...
              <p className="mt-4 text-base leading-relaxed text-ink/70">
                Prices shown are per person for a typical week on this boat. The exact fare depends on the route — pick a
                date and you will see it before anything is confirmed.
              </p>
            </div>
            <a
              href="#dates"
          ...
          <div className="mt-10 rounded-3xl bg-white p-6 lg:p-8">
            ...
            <p className="mt-5 text-xs leading-relaxed text-ink/50">A measured deck plan is available on request — the office will email the PDF.</p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-8xl px-5 py-14 sm:px-6 lg:px-8 lg:py-20">
        <div className="max-w-xl">
          <p className="font-mark text-eyebrow uppercase text-flame">Where she sails</p>
          <h2 className="mt-4 font-display text-3xl font-light leading-tight tracking-tight text-ink-700 sm:text-4xl">
            {trips.length} routes on {boat.name}
          </h2>
        </div>
        <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6">
          {trips.map((t) => (
            <TripCard key={t.slug} trip={t} hideBoat />
          ))}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-8xl px-5 py-14 sm:px-6 lg:px-8 lg:py-20">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div className="max-w-xl">
              ...
              <h2 className="mt-4 font-display text-3xl font-light leading-tight tracking-tight sm:text-4xl">Sailing on {boat.name}</h2>
            </div>
            <Link
              href={routes.departures({ boat: boat.slug })}
